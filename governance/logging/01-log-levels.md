...
| **Information** | Fluxo normal de execução — entrada e saída de métodos, chamadas a serviços externos, mapeamentos, início e fim de iterações. Este é o nível padrão do storytelling. | `logger.LogInformation("[PokemonGetEndpoint][Get] Processar requisicao GET /pokemon/{PokemonId}", id);` |
