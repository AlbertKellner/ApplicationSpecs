...
O projeto valida o comportamento de logging através de um `FakeLogger<T>` que captura todos os logs emitidos durante a execução de testes unitários. Os testes verificam que o padrão de Logging Estruturado com Storytelling é seguido: tipo do evento (nível de log) e conteúdo parcial da mensagem via `Contains`.
